...
|---|---|---|---|
| R01 — Sequestro de conta de cliente (credential stuffing) | Logs de autenticação | Muitas tentativas de login malsucedidas seguidas para a mesma conta em curto intervalo, ou login bem-sucedido a partir de um dispositivo/local nunca usado por aquele usuário imediatamente após tentativas falhas | Bloquear temporariamente novas tentativas de login para a conta, notificar o usuário por e-mail e exigir confirmação adicional (segundo fator) no próximo acesso |
| R12 — Escalonamento de privilégio via API de pedidos (reembolso indevido) | Logs de auditoria de aprovação de reembolso | Reembolso aprovado por uma conta cuja role não é administrador, ou volume de reembolsos aprovados pela mesma conta administrativa muito acima da média em curto período | Suspender temporariamente a conta envolvida, colocar o reembolso em análise manual antes de efetivá-lo e alertar a equipe de segurança |
| R09 — Sobrecarga do checkout (Denial of Service) | Logs de requisições do servidor/API (taxa de requisições por IP) | Volume de requisições ao endpoint de checkout muito acima da média histórica, originado de um mesmo IP ou de um pequeno conjunto de IPs, em um intervalo curto | Acionar limitação de requisições (rate limiting) para os IPs envolvidos, alertar a equipe de infraestrutura e monitorar a disponibilidade do checkout até a normalização |
